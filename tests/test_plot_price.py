...
def test_plot_price():
    ...
    helper_data = pd.DataFrame({
        'Symbol': np.array(['Bitcoin', 'Bitcoin', 'Bitcoin', 'Bitcoin' ,'Bitcoin']),
    ...
    assert helper_plot.encoding.x.shorthand == 'Date', 'x_axis should be mapped to the x axis'
    assert helper_plot.encoding.y.shorthand == 'Close', 'y_axis should be mapped to the y axis'
